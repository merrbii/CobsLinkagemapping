...
circos.par("track.height"=0.4, gap.degree=c(.2, .2, .2, .2, .2,
                                            ...
                                            .2, .2, .2, .2, .2,
                                            .2, .2, .2, .2, .2,
                                            .2, .2, .2, 10), cell.padding=c(0, 0, 0, 0), start.degree = 90, track.margin =c(0.01, 0.01))
...
circos.track(factors=rr$chr, x=rr$start, y=rr$value1, track.margin = c(0.005,0.01), panel.fun=function(x, y) {
  ...
  circos.lines(x, y, col="blue", lwd=1.5, area = F, type="l",border = "blue")
...
data.TT[,4] <- rectcols
tktransparency <- 1
circos.genomicTrackPlotRegion(data.TT, ylim=c(0,1), track.margin = c(0.0001,0), track.height =0.01,
                              bg.col = c("NA","NA","NA","NA","NA","NA","NA","NA","NA","NA","NA","NA","NA","NA","NA","NA","NA","NA","NA","NA",
                                         "NA","NA","NA","NA","NA","NA","NA","NA","NA"), 
                              bg.border = NA, panel.fun = function(region,value,...){
                                circos.genomicRect(region, value, col=adjustcolor(value[[1]],alpha.f = tktransparency), border = NA, ...)	  
...
# Add the track for TE content
circos.track(factors=te$chr, x=te$start, y=te$value1, panel.fun=function(x, y) {
  circos.segments(x0=0, x1=get.cell.meta.data("xlim"), y0=0.3, y1=0.3, lwd=0.1, col="grey")
  circos.segments(x0=0, x1=get.cell.meta.data("xlim"), y0=0.6, y1=0.6, lwd=0.1, col="grey")
  circos.barplot(pos = x, value = y, border = ifelse(y == 0.033268| y == 0.060396| y == 0.100140| y == 0.102896|
                                                       y == 0.104324| y == 0.106176| y == 0.122968| y == 0.133508|
                                                       y == 0.147812| y == 0.168088| y == 0.180732| y == 0.199108|
                                                       y == 0.205504| y == 0.221784| y == 0.225752| y == 0.233328|
                                                       y == 0.243376| y == 0.269300| y >= 0.311552, "orange", "#0075dc"),
                 col= ifelse(y == 0.033268| y == 0.060396| y == 0.100140| y == 0.102896|
                               y == 0.104324| y == 0.106176| y == 0.122968| y == 0.133508|
                               y == 0.147812| y == 0.168088| y == 0.180732| y == 0.199108|
                               y == 0.205504| y == 0.221784| y == 0.225752| y == 0.233328|
                               y == 0.243376| y == 0.269300| y >= 0.311552, "orange", "#0075dc"), lwd = 2)
}, ylim=range(te$value1, na.rm = T), track.height=0.09, bg.border=T, bg.lwd = 0.2)
circos.yaxis(at=c(0.3, 0.6), sector.index = 1, labels.cex=.4, lwd=0, tick.length=0, labels.col=col_text, col="#FFFFFF")
...
circos.track(factors=gene$chr, x=(gene$start+gene$end)*0.5, y=gene$value1, panel.fun=function(x, y) {
  ...
  circos.barplot(pos = x, value = y, border = "limegreen", col="limegreen", lwd = 2)
}, ylim=range(gene$value1), track.height=.09, bg.border=T, bg.lwd = 0.2)
# gene y axis
...
}, ylim=range(c(log10(pii$value1),log10(piu$value1)), na.rm = T), track.height=0.1, bg.border=T, bg.lwd = 0.2)
# pi y axis
...
data <- mutate(data, candidate = ifelse(data$CHR == 1 & data$start >= 6500000 & data$end <=11750000|
                                          data$CHR == 4 & data$start >= 3500000 & data$end <= 10000000|
                                          data$CHR == 6 |
                                          data$CHR == 8 & data$start >= 1750000 & data$end <= 5500000|
                                          data$CHR == 12, "= 0", "> 0"))
...
dev.print(pdf,"~/sciebo/My_PhD/Cobs3.1/linkageMap/manuscript/GenomeResearchsubmission/Revision/MainMaterial/innedRR.Pi.TajD.Fst.Dxy.Pisyn.Pinonsyn.fig2_v.pdf",width=8,height=6)
